...
abstract class LocalHistoryRoute<T> extends Route<T> {
  ...
  ///     TransparentModalRoute.of(context).addLocalHistoryEntry(
  ///         LocalHistoryEntry(
  ///             onRemove: () {
...
class _ModalScope<T> extends StatefulWidget {
  ...
  final TransparentModalRoute<T> route;
...
class _ModalScopeState<T> extends State<_ModalScope<T>> {
  ...
  // something like a TransparentModalRoute.of() dependency triggers an update.
  Widget _page;
...
///
/// [TransparentModalRoute]s cover the entire [Navigator]. They are not necessarily
/// [opaque], however; for example, a pop-up menu uses a [TransparentModalRoute] but only
...
abstract class TransparentModalRoute<T> extends TransitionRoute<T> with LocalHistoryRoute<T> {
  /// Creates a route that blocks interaction with previous routes.
  TransparentModalRoute({
    RouteSettings settings,
  ...
  ///
  /// ```dart
  /// TransparentModalRoute route = TransparentModalRoute.of(context);
  /// ```
  ///
  /// The given [BuildContext] will be rebuilt if the state of the route changes
  /// (specifically, if [isCurrent] or [canPop] change value).
  static TransparentModalRoute<T> of<T extends Object>(BuildContext context) {
  ...
  ///
  /// Whenever you need to change internal state for a [TransparentModalRoute] object, make
  ...
  static RoutePredicate withName(String name) {
    return (Route<dynamic> route) {
      return !route.willHandlePopInternally
          && route is TransparentModalRoute
  ...
  /// route's state changes unless it uses [TransparentModalRoute.of]. For a builder that
  /// is called every time the route's state changes, consider
  /// [buildTransitions]. For widgets that change their behavior when the
  /// route's state changes, consider [TransparentModalRoute.of] to obtain a reference to
  ...
  /// for TransparentModalRoute descendants to collectively define the value of `willPop`.
  ///
  /// See also:
  ///
  ...
  /// widget finds the enclosing [TransparentModalRoute] and uses this function to register
  /// this callback:
  ///
  /// ```dart
  /// Widget build(BuildContext context) {
  ...
  ///
  /// To register a callback manually, look up the enclosing [TransparentModalRoute] in a
  /// [State.didChangeDependencies] callback:
  ///
  /// ```dart
  /// TransparentModalRoute<dynamic> _route;
  ///
  /// @override
  /// void didChangeDependencies() {
  ///  super.didChangeDependencies();
  ///  _route?.removeScopedWillPopCallback(askTheUserIfTheyAreSure);
  ///  _route = TransparentModalRoute.of(context);
...
//    _modalBarrier.markNeedsBuild();
  ...
  /// [TransparentModalRoute.of] will be notified.
...
abstract class TransparentPopupRoute<T> extends TransparentModalRoute<T> {
  /// Initializes the [PopupRoute].
  TransparentPopupRoute({
    RouteSettings settings,
  }) : super(settings: settings);
...
///   void didChangeDependencies() {
///     super.didChangeDependencies();
///     routeObserver.subscribe(this, TransparentModalRoute.of(context));
///   }
///
///   @override
///   void dispose() {
...
class _TransparentDialogRoute<T> extends PopupRoute<T> {
  _TransparentDialogRoute({
...
Future<T> showGeneralDialog<T>({
  ...
  return Navigator.of(context, rootNavigator: true).push(_TransparentDialogRoute<T>(
...
///
/// See [TransparentModalRoute.buildPage] for complete definition of the parameters.
...
/// Used in [PageRouteBuilder] and [showGeneralDialog].
///
/// See [TransparentModalRoute.buildTransitions] for complete definition of the parameters.
